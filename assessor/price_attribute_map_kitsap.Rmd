...
dlist$Residential_5_yr_dwelling_sales$Sale_Dt <- lubridate::mdy(dlist$Residential_5_yr_dwelling_sales$Sale_Dt)
dlist$Parcels$property_class <- as.character(dlist$Parcels$property_class)
```
...
* Housing type descriptions from Residential_5_yr_dwelling_sales are similiar to those found in Dwellings with a few additional fields (likely MH types from MH table)
```{r}
...
assemble.housing.price.dataset <- function() {
  ...
  prcl[dlist$codes[Field_name == "property_class"], on = c("property_class" = "Code"), Description := i.Description]
  sales[prcl, on = c("LRSN" = "rp_acct_id", "Acct_no" = "acct_no"), Description := i.Description]
}
