...
#in the denominator we will need to have a number of all households in the region - either way would work
#option 1
...
#option 2
days_hh_weight_2 = days %>% group_by(hhid,daynum) %>% slice(which.max(hh_day_wt_combined))
sum(shop_trip_per_hh$weight_comb)/sum(days_hh_weight_2$hh_day_wt_combined)
...
person_day_income <- hh_days %>% group_by(hhincome_detailed) %>%
  summarise(n=n(), day_combined = sum(hh_wt_combined.x))

person_day_income2 <- hh_days2 %>% group_by(hhincome_detailed) %>%
...
#household table joined with day_household (identification whatever delivery was received or not)

hh_delivery = left_join(day_household_2,hh, by = c("hhid" = "hhid"))



#trips table limited to trip weights and grouped by household id
shopping_trips_by_hh = hh_shopping_join %>% group_by(hhid) %>% summarise(trip_wt_combined = sum(trip_wt_combined * shop_trip))
...
deliv_joined_shop_trips = left_join(day_household_2,shopping_trips_by_hh,  by = c("hhid" = "hhid")) 


#if hh received delivery or not with shopping trips

shop_trips_delivery = deliv_joined_shop_trips %>%
  filter(day_wts > 0) %>% 
  group_by(delivery) %>% 
  summarise(n=n(),sum_wt_comb = sum(trip_wt_combined, na.rm = TRUE)) %>% 
  mutate(perc_comb = sum_wt_comb/sum(sum_wt_comb)*100) %>% 
  ungroup() %>%  mutate(MOE=z*(p_MOE/sum(n))^(1/2)*100)
...
person_day_delivery <- day_household_2 %>% group_by(delivery) %>%
  summarise(n=n(), day_combined = sum(day_wts))
...
delivery_income = hh_delivery %>% 
  group_by(delivery,hhincome_broad) %>% 
  summarise(n=n(),sum_wt_comb = sum(day_wts)) %>% 
  mutate(perc_comb = sum_wt_comb/sum(sum_wt_comb)*100)
