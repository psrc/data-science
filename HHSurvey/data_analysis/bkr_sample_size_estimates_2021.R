...
sql.trip.query <- paste("SELECT origin_parcel_dim_id, 'dest_parcel_dim_id' 'mode_simple', 'trip_weight_2019_v2021' FROM HHSurvey.v_trips")
trips <- read.dt(sql.trip.query, 'sqlquery')

small.areas<-paste("SELECT parcel_dim_id, city_name_2017 FROM small_areas.parcel_dim")
