...
flu_dissim<- flu_dissim %>%mutate(median_max_du_capped=replace(median_max_du,median_max_du>100,100))
...
create_map_du <- function(c.layer,
  ...
  pal <- leaflet::colorNumeric(palette="Purples", domain = c.layer$median_max_du_capped)
  
  labels <- paste0(
                   'Median Max Du per Acre: ',c.layer$median_max_du_capped) %>% lapply(htmltools::HTML)
  ...
  m <- leaflet::leaflet() %>%
    ...
    leaflet::addPolygons(data=c.layer,
                         fillOpacity = 0.7,
                         fillColor = pal(c.layer$median_max_du_capped),
                         opacity = 0,
                         stroke=FALSE,
                         weight = 0.7,
                         color = "#BCBEC0",
    ...
    leaflet::addLegend(pal = pal,
                       values = c.layer$median_max_du_capped,
...
c.layer$median_max_du_capped[is.na(c.layer$median_max_du_capped)]<-0
