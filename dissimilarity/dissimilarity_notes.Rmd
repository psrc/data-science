...
```
Now let's map the range of Max DU per acre.Red shows where there is more restriction on max du per acre, Blue shows where there is less restriction.
...
A MAP OF MAX DU Greater or Less than 10. 
```{r du_10}
...
```
A MAP OF MAX DU Greater or Less than 25.
```{r, echo=FALSE}
...
WHITE-BLACK DISSIMILIARITY MAP
```{r, echo=FALSE}
...
flu_dissim_seattle %>% group_by(sf_mf_cats) %>% summarise(mean_dissim_white_black=mean(White_Black_Dissim))
...
ggplot(flu_dissim_seattle, aes(x=mean_max_du_capped, y=White_Black_Dissim))+
  geom_point()+
  geom_smooth(method=lm, formula='y ~poly(x,3)')
cor.test(flu_dissim_seattle$White_Black_Dissim, flu_dissim_seattle$mean_max_du,
         method = "spearman")
...
anova_sf_white_black = aov(White_Black_Dissim ~is_sf_low, data=flu_dissim_seattle)
anova_sf_white_minority = aov(White_Minority_Dissim ~is_sf_low, data=flu_dissim_seattle)
anova_sf_white_aian = aov(White_AIAN_Dissim ~is_sf_low, data=flu_dissim_seattle)
summary(anova_sf_white_aian)
summary(anova_sf_white_minority)
summary(anova_sf_white_black)
```
...
```{r seattle, echo=FALSE}
sf_threshold_high<-25



# Add a variable for defining single family as greater than 0, but less than some max DU per acre
flu_dissim_seattle<-flu_dissim_seattle %>% mutate(
  is_sf_high = case_when(
    max_max_du>=sf_threshold_high ~ 0,
    max_max_du<sf_threshold_high~ 1
  ))%>%
  mutate(is_sf_high= as.numeric(is_sf_high))
...
flu_dissim_seattle%>%
  group_by(is_sf_high)%>%
  summarise(mean_white_black_dissim_high=mean(White_Black_Dissim))
```
