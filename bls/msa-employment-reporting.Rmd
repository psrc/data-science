...
In 2017, there were a bunch of jobs in the state .......
...
# Trim down the datatables to only include the FIPS code and Employment Totals
statewide_jobs <- wa_previous_year[,.(area_fips,value)]
names(statewide_jobs)[names(statewide_jobs)=="value"] <- "previous_year_jobs"
total_state_jobs <- sum(statewide_jobs$previous_year_jobs)
statewide_jobs$state_share <- (statewide_jobs$previous_year_jobs / total_state_jobs)*100

# Washington State County Shapefile
...
rng <- range(county_jobs$state_share)
max_bin <- max(abs(rng))
...
bins <- c(0, breaks)
pal <- colorBin("Blues", domain = county_jobs$state_share, bins = bins)

# Create a label format for the interactive map
labels <- paste0("<b>","County: ", "</b>",county_jobs$COUNTY_NM,
                 "<b> <br>","Jobs: ", "</b>", prettyNum(round(county_jobs$previous_year_jobs, -1), big.mark = ","),
                 "<b> <br>","Share: ", "</b>", prettyNum(round(county_jobs$state_share, 1), big.mark = ","),"%") %>% lapply(htmltools::HTML)

county_map <- leaflet(data = county_jobs) %>% 
  addTiles() %>%
  addPolygons(fillColor = pal(county_jobs$state_share),
              weight = 2,
              opacity = 1,
              color = "white",
              dashArray = "3",
              ...
              labelOptions = labelOptions(
                ...
                textsize = "15px",
                direction = "auto"))%>%
  addLegend("bottomright", pal=pal, values = county_jobs$state_share,
            title = "Share of Jobs",
...
In 2018, there were even more jobs in the state .......
...
wa_monthly_jobs <- washington_data[attribute == 'jobs']
wa_latest_year <- wa_monthly_jobs[date %in% current_date]

# Trim down the datatables to only include the FIPS code and Employment Totals
statewide_jobs <- wa_latest_year[,.(area_fips,value)]
names(statewide_jobs)[names(statewide_jobs)=="value"] <- "latest_year_jobs"
total_state_jobs <- sum(statewide_jobs$latest_year_jobs)
statewide_jobs$state_share <- (statewide_jobs$latest_year_jobs / total_state_jobs)*100
...
# Create a label format for the interactive map
labels <- paste0("<b>","County: ", "</b>",county_jobs$COUNTY_NM,
                 "<b> <br>","Jobs: ", "</b>", prettyNum(round(county_jobs$latest_year_jobs, -1), big.mark = ","),
                 "<b> <br>","Share: ", "</b>", prettyNum(round(county_jobs$state_share, 1), big.mark = ","),"%") %>% lapply(htmltools::HTML)
